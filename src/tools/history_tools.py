...
@tool("get_booking_history",args_schema={"limit": int})
@handle_errors(error_class=ToolError)
def get_booking_history(runtime: ToolRuntime,limit: int = Limits.HISTORY_DEFAULT_LIMIT) -> dict:
    """
...
@handle_errors(error_class=ToolError)
def get_last_booking(runtime:ToolRuntime) -> dict:
    """
...
@handle_errors(error_class=ToolError)
def get_bookings_by_status(status: str,runtime:ToolRuntime) -> dict:
    """
